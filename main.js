...
(function () {
  ...
  /* ------------------------------------------
     LASER LIGHTS
     Glow restored — blur only on the wide
     outer glow layer (once per beam), not
     on every stroke. Core beam has no blur.
     Loop throttled to 40fps max.
  ------------------------------------------ */
  function rgba(c, a) {
  ...
  function drawBeams(ctx, W, H, t, shift) {
    BEAMS.forEach(function (b, i) {
      ...
      /* outer glow — blur applied once per beam only */
      ctx.save();
      ctx.filter = 'blur(7px)';
      ctx.globalAlpha = .45;
  ...
  function loop(ts) {
    ...
    if (x1 && W1 > 0 && H1 > 0) {
      ...
      bg1.addColorStop(0, 'rgba(170,255,0,.05)');
      bg1.addColorStop(1, 'transparent');
      x1.fillStyle   = bg1;
      x1.globalAlpha = 1;
      x1.fillRect(0, 0, W1, H1);
    ...
    if (x2 && W2 > 0 && H2 > 0) {
      ...
      bg2.addColorStop(0, 'rgba(170,255,0,.05)');
